extends BaseEnemy
## Pyro Technician — ranged fire shooter. Maintains distance, fires fire-element projectiles.
...
func _ready() -> void:
	if enemy_data == null:
		...
		enemy_data.enemy_name = "Pyro Technician"
		enemy_data.enemy_type = EnemyData.EnemyType.PYRO_TECH
		enemy_data.max_health = 25.0
		enemy_data.move_speed = 130.0
	...
	if ObjectPool.get_pool_size("pyrotech_projectiles") == 0:
		ObjectPool.preload_pool("pyrotech_projectiles", _projectile_scene, 15)
...
func _perform_attack() -> void:
	if _target == null:
		return
	var proj: Node2D = ObjectPool.get_instance("pyrotech_projectiles") as Node2D
	if proj == null:
		return
	...
	weapon.element = WeaponData.Element.FIRE
