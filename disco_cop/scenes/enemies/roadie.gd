extends BaseEnemy
## Roadie — slow melee tank. Charges during chase, high knockback, self-stuns after attack.

const CHARGE_ACCEL := 300.0
const MAX_CHARGE_SPEED := 140.0
...
func _ready() -> void:
	if enemy_data == null:
		...
		enemy_data.enemy_name = "Roadie"
		enemy_data.enemy_type = EnemyData.EnemyType.ROADIE
		enemy_data.max_health = 80.0
		enemy_data.move_speed = 90.0
		enemy_data.damage = 25.0
		enemy_data.attack_range = 50.0
...
func _perform_attack() -> void:
	if _target and _target.has_method("take_damage"):
		...
		if _target.has_method("apply_knockback"):
			...
			_target.apply_knockback(Vector2(kb_dir * 200.0, -80.0))
